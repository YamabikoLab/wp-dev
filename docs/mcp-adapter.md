...
MCP Adapter is disabled by default. It is installed only when the environment explicitly sets `MCP_ADAPTER_ENABLED=true` and the running WordPress version is 6.9 or later.

## Enable MCP Adapter

The `default` environment uses WordPress 7.0.2 and can enable MCP Adapter. In `environments/default.env`, set:
...
MCP Adapter requires WordPress 6.9 or later.

- `default` currently uses WordPress 7.0.2 and supports MCP Adapter.
