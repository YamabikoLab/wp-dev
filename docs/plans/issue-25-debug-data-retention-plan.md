...
本プランは実装順序と設計境界を定める。PR #46 の実装がこのブランチへ取り込まれているため、現在の PR #45 には本プラン文書だけでなく、本プランに基づく設定変更と運用ドキュメントの実装も含まれる。
...
PR #46 の取り込みにより、以下のフェーズに対応する実装変更も現在の PR #45 に含まれている。実環境での最終確認は、Issue #25 の検証項目に沿って別途行う。
